        ...
        public static void Send()
        {
            ...
            { 
                ...
                using (IModel channel = connection.CreateModel())
                {
                    ...
                    channel.ExchangeDeclare("ss", "direct");
                    channel.QueueBind("WebLog", "ss", "web");
                    channel.QueueBind("PClOG", "ss", "pc");
                   
                    string[] routtekeys = { "web", "pc" };
                    for (int i = 0; i < 3000000; i++)
                    {
                        Thread.Sleep(100);
                        var tempindex = i % 2;
                        var tempmsg = i + message;
                        channel.BasicPublish(
                        exchange: "ss",
                        routingKey: routtekeys[tempindex],
                        basicProperties: null,
                        body: Encoding.UTF8.GetBytes(tempmsg));
                        Console.WriteLine(string.Format("我是生产者，我发送了一条消息{0}", tempmsg));
                    }
